ARM CM4 & cxd32xx 在 NuttX12.4.0 上的移植案例
==========================================
...
修改 \"arch/arm/Kconfig\" 以注册 SoC 目录。

为 SoC 目录添加 \"Kconfig\" 和 \"Make.defs\"。
...
有关这些文件的详细信息，请参阅 `apache/nuttx/pull/12441`_ 

步骤2：添加开发板
---------------------

添加开发板目录有两种方式。一种是 in-tree（树内），另一种是 out-of-tree（树外）。
有关详细信息，请参阅 :doc:`/guides/customboards`。
如果您想添加自己的应用程序，请参阅 :doc:`/guides/customapps`。

本次选择 out-of-tree 方式。目录结构如下：
...
添加 \"Kconfig\"、\"defconfig\"、\"Make.defs\" 和 \"Makefile\"。
...
有关这些文件的详细信息，请参阅 :ref:`out-of-tree`。

步骤3：配置
-----------------

现在您可以使用以下命令进行配置。确认这些命令能够正常工作非常重要。
...
您可以使用以下命令进行 menuconfig。

.. code-block::

  $ make menuconfig

您可以使用以下命令进行 distclean。
...
尝试使用步骤1-2 的实现进行构建，如果您只实现了步骤1-2 中的目录 / Kconfig / Make.defs / Makefile / defconfig，
您将遇到以下错误。
...
尝试参考 cxd56xx 添加 \"irq.h\" 并重新构建。这次构建系统会告诉您需要实现 \"chip.h\"，如下所示。
...
编译通过后，您将遇到以下错误。要解决此错误，您必须实现链接脚本。
...
尝试参考某些开发板（cxd56xx、lpc43xx 等）或 :ref:`out-of-tree` 添加链接脚本并重新构建，
构建系统会告诉您需要实现一些符号，如下所示。
...
有一些实现项目，主要项目如下所列。为了通过链接，请尝试实现这些符号。

+-----------------+------------------------+--------------------------------------------------------------------+
| 类别            | 项目                   | 说明                                                               |
+-----------------+------------------------+--------------------------------------------------------------------+
| SoC 目录        | 入口点：__start        | `apache/nuttx/pull/12441`_                                         |
+                 +------------------------+                                                                    +
|                 | 串口驱动               |                                                                    |
+                 +------------------------+                                                                    +
|                 | IDLE 任务栈            |                                                                    |
...
|                 | 定时器                 |                                                                    |
+-----------------+------------------------+------------------------------------------------+-------------------+
| 开发板目录      | board_app_initialize   | :doc:`/applications/nsh/customizing`           |:ref:`out-of-tree` |
+                 +------------------------+------------------------------------------------+                   |
...
确认 NuttShell (NSH) 启动后，尝试执行 \"apps/testing/ostest\"。
\"apps/testing/ostest\" 需要以下配置。
...
另外建议再进行一项测试，以检查定时器实现是否能够准确计时。
我使用以下代码进行了检查。关于函数 cxd32_timerget()，
请参阅 `apache/nuttx/pull/12441`_ 

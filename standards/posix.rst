...
Apache NuttX **未**获得使用 POSIX 商标的许可。NuttX 仅将 POSIX
标准作为开发指南。

本页面总结了 NuttX 中支持的 POSIX 功能。
...
.. [#fn1] 不支持 ``fenv.h`` 相关函数。
.. [#fn2] 在较旧的修订版中称为 ``XSI_THREAD_MUTEX_EXT``
          一些接口已从 ``POSIX_THREADS_EXT``
          移至 ``POSIX_THREADS_EXT``。
...
.. [#fn3] NuttX 为 ``_POSIX_MEMLOCK`` 和 ``_POSIX_MEMLOCK_RANGE`` 提供虚拟实现
          这对于没有 MMU 的设备已经足够，
          但不满足带 MMU 的设备的需求。
.. [#fn4] 在 POSIX 新修订版中已弃用。已合并到其他基本选项中。
.. [#fn5] 仅开发平台需要。
          对于 PSE54，此需求也适用于执行平台，
          但不受支持。
...
实时控制器系统配置文件（PSE52）包含 PSE51 的所有功能。
...
.. [#fn6] NuttX 支持不同的 libm 实现，具有不同
          的支持函数集。NuttX 中的 libm 支持由 Kconfig
          选择选项控制。
...
.. [#fn7] ``_POSIX_TRACE*`` 已过时。

POSIX PSE53 - 专用
-----------------------

专用实时系统配置文件（PSE53）包含 PSE52 和 PSE51 的所有功能。
...
.. [#fn8] 在 POSIX 新修订版中已弃用。
          已合并到 ``POSIX_DEVICE_IO``。
...
.. [#fn9] NuttX 为 ``_POSIX_MEMORY_PROTECTION`` 提供虚拟实现
          这对于没有 MMU 的设备已经足够，但不满足
          带 MMU 的设备的需求。

POSIX PSE54 - 多用途
--------------------------

多用途实时系统配置文件（PSE54）包含 PSE53、PSE52 和 PSE51 的所有功能。
...
.. [#fn10] 在 POSIX 新修订版中已弃用。
           已移至 ``POSIX_WIDE_CHAR_DEVICE_IO``。
...
.. [#fn11] NuttX 支持不同的 ``libm`` 实现，具有不同
           的支持函数集。NuttX 中的 ``libm`` 支持由
           Kconfig 选择选项控制。
...
.. [#fn13] 待复查：为什么在 ``unistd.h`` 中 ``#undef  _POSIX_FSYNC``？
...
.. [#fn16] ``_POSIX_TRACE*`` 已过时。

_POSIX_TIMERS
-------------

取决于 ``!CONFIG_DISABLE_POSIX_TIMERS``
